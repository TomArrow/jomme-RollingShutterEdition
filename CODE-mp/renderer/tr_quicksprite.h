...
private:
			...
			unsigned int	mIndexes[SHADER_MAX_VERTEXES*6/4];			// Ideally this would be static, cause it never changes
			...
			vec3_t			mLightDirs[SHADER_MAX_VERTEXES];
...
public:
	...
	virtual ~CQuickSpriteSystem();
			...
			void Add(float *pointdata, color4f_t color, vec3_t lightdir, vec2_t fog=NULL);
};
