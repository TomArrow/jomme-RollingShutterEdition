...
extern float byteToLinearTable[256];

inline float R_sRGBToLinear(const float n) {
	return (n > 0.04045f ? (float)pow((n + 0.055) / 1.055, 2.4) : n / 12.92f);
}
inline float R_sRGBToLinear(const byte n) {
	return byteToLinearTable[n];
}
...
typedef struct {
	...
	int		  (*GetMd3Verts)( qhandle_t handle, const char* surfaceName, meshTriangle_t* meshTris, int bufferSizeTris );
	...
	void	(*AddWindPointToScene)( const vec3_t origin, const vec3_t direction, float intensity, float radius);
	int		(*AddViewToScene)( const vec3_t origin, qboolean is360, qboolean copyAxis, const float* axis, int flags);
