...
qboolean R_MME_JitterOrigin( float *x, float *y ) {
	...
	*y = 0;
	...
	if (r_fboGLSL->integer && ENABLEGLSL && !r_fboGLSLOff->integer && r_fboFishEye->integer) {
...
void R_MME_JitterView( float *pixels, float *eyes, float * voxelshadowlights, float* dlights, int* blurJitterIndex, int* jitterTotalFrames) {
	...
	*jitterTotalFrames = 0;
	...
	if (tr.captureIsActive &&  passData.quickJitter) {
		...
		scale *= radius * R_MME_FocusScale(focus);
		if (r_fboGLSL->integer && ENABLEGLSL && !r_fboGLSLOff->integer && r_fboFishEye->integer) scale = 1.0f;
		if (mme_dofQuickRandom->integer == 3 && passData.superRandomDofJitterControl.superRandomPotentialJitterPositions) {
			superRandomDofJitterControl_t* srJControl = &passData.superRandomDofJitterControl;
