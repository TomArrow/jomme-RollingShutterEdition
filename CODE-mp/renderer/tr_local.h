...
typedef struct {
	...
	float fovY;
	int		jitterIndex;
	int		jitterTotalFrames;
	qboolean tessellationActive;
	float texAverageBrightness;
	bool isLightmap;
	bool isWorldBrush;
	bool isSaber;
	int renderFlags; // 1 = simple lighting, e.g. surfacesprites, 2 = no lighting (z prepass), 4 = twosided
	int alphaFunc;
...
qboolean R_FrameBuffer_SetDynamicUniforms3(); // just set in fboUniformsEx and call this
