...
static void CG_AddExplosion( localEntity_t *ex ) {
	...
	if ( ex->light ) {
		...
		trap_R_AddLightToScene(ent->origin, light, ex->lightColor[0], ex->lightColor[1], ex->lightColor[2], 0.0f, qfalse);
	}
}
...
static void CG_AddSpriteExplosion( localEntity_t *le ) {
	...
	if ( le->light ) {
		...
		trap_R_AddLightToScene(re.origin, light, le->lightColor[0], le->lightColor[1], le->lightColor[2], 0.0f, qfalse);
	}
}
